...
using namespace std;
using namespace sf;
String TileMap[H] = {}; // рабочий массив для обработки игрой

/*
        В данной игре мы создаем несколько массивов с уровнями, и один пустой массив, который 
    и будет обрабатываться игрой(Проверки на столкновения с ГГ, врагами, разрушение блоков,
    сбор и генерация монеток).
        При выборе определенного уровня или переходе на следующий, в рабочий массив будет копироваться 
    один из заготовленных уровней - это унифицирует код и защищает от изменения оригиналы уровней
*/ 
